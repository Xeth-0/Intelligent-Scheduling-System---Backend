...
MAX_DURATION_SECONDS = 0.5  # Increased for potentially longer runs
...
class GeneticSchedulerEnhanced:
    """
    Enhanced genetic scheduler with multi-objective fitness evaluation.
    Backward compatible with single-objective optimization while supporting
    the new detailed fitness system.
    """
    ...
    def run(
        ...
        """
        Enhanced run method that returns the best solution, its fitness score,
        and detailed fitness report.
        """
    ...
    def initialize_chromosome(self) -> List[ScheduledItem]:
        """Keep existing implementation but fix the bug."""
        ...
        for base_gene in self.base_chromosome:
            ...
            chromosome.append(new_gene)  # Fix: append new_gene, not base_gene
        return chromosome
    ...
    def selection(
        ...
        # Keep existing tournament selection
    ...
    def fitness(self, chromosome: List[ScheduledItem]) -> float:
        """Keep original fitness method for backward compatibility."""
        # This is your original fitness function - keeping it for fallback
        # [Original fitness implementation would go here]
        # For now, let's use the new evaluator and extract a single score
        report = self.fitness_evaluator.evaluate(chromosome)
